...
module.exports = {
  ...
  const effects = { hi_mult: 1, extra_luck: 0, raw: [] };
      for (const row of res.rows) {
        ...
        if (row.item === 'cavendish') {
          // permanent ability: mark presence for consumers
          effects.cavendish = true;
        }
        if (row.item === 'predictor') {
          effects.predictor = true;
        }
  ...
  const MAX_EXTRA_LUCK = 100; // maximum extra_luck percentage points
